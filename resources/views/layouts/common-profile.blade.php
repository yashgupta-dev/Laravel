						...
						<li class="nav-item dropdown nav-notifications">
							...
							<div class="dropdown-menu" aria-labelledby="notificationDropdown">
								<div class="dropdown-header d-flex align-items-center justify-content-between">
									<p id="count" class="mb-0 font-weight-medium" data-count="{{ count(auth()->user()->unreadNotifications) }}">
										@if(!count(auth()->user()->unreadNotifications))
											{{ __('No Notifications') }}
										@else
											{{ count(auth()->user()->unreadNotifications) }} {{ __(' New Notifications') }}
										@endif
									</p>
									@if(count(auth()->user()->unreadNotifications))
										<a href="javascript:;" class="text-muted" id="clear-all">{{ __('Clear all') }}</a>
									@endif
								</div>
								<div class="dropdown-body" @if(count(auth()->user()->unreadNotifications) >= 5) style="height:400px;overflow-y:scroll;" @endif>
						...
						<li class="nav-item dropdown nav-profile">
							<a class="nav-link dropdown-toggle" href="javascript:;" id="profileDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
								@if(!in_array(Auth::user()->profile,['0','1','2','3','4']))
									<img src="{{ asset('storage') }}/profile/{{ explode('/',Auth::user()->profile)[2] }}" alt="{{ Auth::user()->name }}" style="object-fit: cover;">
								@else
									<img src="{{ asset('logo') }}/{{ Auth::user()->profile }}.png" alt="{{ Auth::user()->name }}">
								@endif
							</a>
							<div class="dropdown-menu" aria-labelledby="profileDropdown">
								<div class="dropdown-body">
									<ul class="profile-nav p-0">
										...
										@if(!empty(json_decode($access)->access) && in_array('User/ProfileController',json_decode($access)->access)) 
                                        <li class="nav-item">
